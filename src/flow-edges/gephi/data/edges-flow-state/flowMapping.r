...
draw.map <- function(year, ylim=c(0,85)) {
  ...
  ggplot(state.map.df, aes(x=long, y=lat, group=group)) + # map_data("state"), map_data("world", "brazil")
...
for (year in 1970:2013) {
  ...
  path <- "~/Documents/code/github/lucachaves/lattesGephi/src/flow-edges/gephi/data/edges-flow-state/"
  file_map <- paste(path,"img-map-", theme, "-normal/map-",year,".png", sep="")
  file_data <- paste(path,"data-latlon/lattes-flows-state-", year,".csv", sep="")
