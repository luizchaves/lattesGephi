...
R.eval <<END
	...
	library(reshape2)
	library(scales)
	flow <- read.csv("~/Documents/code/github/lucachaves/lattesGephi/src/flow-edges/gephi/data/edges-flow-region/edges-flow-region-distance-year-#{year}.csv", sep=",",header=T, check.names = FALSE)
	...
	ggsave(filename="~/Documents/code/github/lucachaves/lattesGephi/src/flow-edges/gephi/data/edges-flow-region/edges-flow-region-distance-year-#{year}#{sufix}.png", plot=p, width=14, height=10, dpi=300)
END
else
R.eval <<END
	library(ggplot2)
...
file = File.read('edges-flow-region-distance-year.csv')
...
flow = []
# names2 = {}
names = {"1"=>"sul", "2"=>"sudeste", "3"=>"centro-oeste", "4"=>"nordeste", "5"=>"norte"}
...
years.each{|year|
	...
	File.write("edges-flow-region/edges-flow-region-distance-year-#{year}.csv", csv_string)
	...
	File.write("edges-flow-region/edges-flow-region-distance-year-#{year}2.csv", csv_string)
}
years.each{|year|
	if kind_change == nil
...
File.write("edges-flow-region/edges-flow-region-distance-years.csv", csv_string)
csv_string.gsub!(",","\";\"")
csv_string.gsub!("\n","\"\n\"")
csv_string = "\"#{csv_string}\""
csv_string.gsub!(".",",")
File.write("edges-flow-region/edges-flow-region-distance-years2.csv", csv_string)

puts "######## Create animation"
# `convert -delay 300 -loop 0 edges-flow-region/edges-flow-region-distance-year-*.png edges-flow-region/animaion.gif`
puts "fim"