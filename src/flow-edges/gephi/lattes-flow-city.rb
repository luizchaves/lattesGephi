...
edges.each{|edge|
	...
	id = source[14]+"-"+source[15]+"-"+target[14]+"-"+target[15]+"-"+kind
...
edges_clean.each{|index, edge|
	...
	source = places[source[14]+source[15]][:id]
	target = edge[:target]
	target_kind = target[2]
	target = places[target[14]+target[15]][:id]
...
csv_string = CSV.generate(:col_sep => ",") do |csv|
	...
	places.each{|index,place|
		...
		csv << [place[:id],place[:city],nil,place[:city],place[:state],place[:country],place[:latitude],place[:longitude]]
	}
end
File.write("data/nodes-flow-city.csv", csv_string)
...
File.write("data/edges-flow-city.csv", csv_string)
