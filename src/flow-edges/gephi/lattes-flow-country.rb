...
locations.each{|loc|
	...
	if places[country].nil?
		countNode += 1
		places[country] = {id: countNode, city: loc[9], state: loc[10], country: loc[12], latitude: loc[14], longitude: loc[15]} 
	end
}
...
File.write("nodes-flow-country.csv", csv_string)
...
edges_clean.each{|index, edge|
	...
	source = places[source[12]][:id]
	target = edge[:target]
	target_kind = target[2]
	target = places[target[12]][:id]
...
File.write("edges-flow-country.csv", csv_string)
